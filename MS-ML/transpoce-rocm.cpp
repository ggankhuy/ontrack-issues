...
#define NUM (size_t(4)*1024*1024*1024)

typedef std::chrono::high_resolution_clock Clock;

decltype(auto) duration(std::chrono::time_point<Clock> start, std::chrono::time_point<Clock> end) {
...
int main(int argc, char **argv) {
  ...
  cout << "Agent: " << devProp.name << endl;

  size_t i;
  int errors;

  hostA = (int16_t*)malloc(sizeof(int16_t)*NUM);
  hostB = (int16_t*)malloc(sizeof(int16_t)*NUM);
  printf("%p %p\n", hostA, hostB);
  fflush(stdout);
  ...
  HIP_ASSERT(hipMalloc((void**)&deviceA, sizeof(int16_t)*NUM));
  HIP_ASSERT(hipMalloc((void**)&deviceB, sizeof(int16_t)*NUM));
  printf("%p %p\n", deviceA, deviceB);
  fflush(stdout);
  
  HIP_ASSERT(hipMemcpy(deviceB, hostB, sizeof(int16_t)*NUM, hipMemcpyHostToDevice));
  ...
  for (int i = 0; i < invocations; i++) {
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(TransposeKernel<int16_t>), dim3(blocksPerGrid), dim3(maxThreadsPerBlock), 0, 0, 
  ...
  HIP_ASSERT(hipMemcpy(hostA, deviceA, NUM*sizeof(int16_t), hipMemcpyDeviceToHost));
  HIP_ASSERT(hipMemcpy(hostB, deviceB, NUM*sizeof(int16_t), hipMemcpyDeviceToHost));
  ...
  for (int k = 0; k < shape[2]; k++) {
	  ...
	  if (hostB[size_t(i)*shape[1]*shape[2] + j*shape[2] + k] != hostA[size_t(j)*shape[0]*shape[2] + i*shape[2] + k])
		  bad = true;
  }
