...
CONFIG_MAGMA_CHECKOUT_ALWAYS=1

if [[ -d magma ]] ; then
    if [[ $CONFIG_MAGMA_CHECKOUT_ALWAYS==1 ]] ; then
        rm -rf magma
        git clone https://bitbucket.org/icl/magma.git
    else 
        echo "Bypassing magma build as directory exists."
    fi
fi
...
echo "$MINICONDA_SRC_DIR/pkgs/mkl-2023.1.0-h213fc3f_46344/lib" | $SUDO tee /etc/ld.so.conf.d/mkl.conf
