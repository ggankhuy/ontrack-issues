...
#  echo "gpgkey=http://repo.radeon.com/rocm/rocm.gpg.key" >> /etc/yum.repos.d/rocm.repo
  ...
  # temporary fix. Dont use the static value.
  yum install -y http://repo.radeon.com/amdgpu-install/22.20/rhel/8.5/amdgpu-install-22.20.50200-1.el8.noarch.rpm
  echo "8.5" > /etc/dnf/vars/amdgpudistro  
  yum --enablerepo=extras
  amdgpu-install --usecase=rocm --no-dkms -y --nobest --allowerasing
  yum update -y
  yum install -y \
                   rocm-dev \
                   rocm-utils \
